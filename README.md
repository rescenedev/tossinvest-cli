...
([FAQ](#faq)). 단, PER·PBR·EPS 같은 **공개 밸류에이션 지표**는 출처(네이버 금융)를 명시한
외부 참고 데이터로 `stock fundamentals` 에서 따로 제공합니다 — 거래·인증 경로와 분리된 읽기
전용이라 토스 계정 리스크와 무관합니다. AI 에이전트(Claude Code 등)에서 도구로 쓰는 규약은
...
toss stock fundamentals 005930 AAPL  # PER·PBR·EPS 등 밸류에이션 (REPL: f 005930)
```

`fundamentals` 는 **네이버 금융의 공개 밸류에이션 지표**(PER·추정PER·PBR·EPS·BPS·배당수익률·시가총액·52주
고저 등)를 가져옵니다 — 국내/해외 모두 지원. 토스 거래·인증 경로와 분리된 **외부 참고 데이터(읽기 전용)**로,
출처를 명시하며 주문에는 절대 사용되지 않습니다 ([FAQ](#faq)).
...
**Q. PER·PBR·EPS 는 어디서 가져오나요? "공식 API 만 쓴다"면서요.**
밸류에이션 지표는 토스 Open API 에 없어 **네이버 금융의 공개 데이터**를 가져옵니다. 단 이는
`stock fundamentals` 전용의 **외부 참고 데이터(읽기 전용)**로, 출처를 항상 명시하고 토스
거래·인증과는 완전히 분리됩니다 — 주문에 쓰이지 않으며 토스 계정 리스크와 무관합니다. 네이버
응답 형식이 바뀌면 깨질 수 있어, 안 되면 친절한 경고만 출력하고 나머지 기능은 그대로 동작합니다.
