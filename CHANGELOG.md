...
### Added
- **펀더멘털 조회** `stock fundamentals 005930 AAPL` (REPL 숏컷 `f`) — PER·추정PER·PBR·EPS·BPS·배당수익률·시가총액·52주 고저 등. 국내/해외 모두 지원(네이버 검색으로 해외 티커 해석). `w` 종목 대시보드에도 핵심 지표(PER·PBR·EPS·배당) 한 줄 추가
- 출처는 **네이버 금융(외부 참고 데이터)** — 토스 거래·인증 경로와 분리된 독립 HTTP·읽기 전용이며, 화면·`--json`·`--csv` 모두 출처를 명시. 네트워크 실패·미상장은 우아하게 경고 처리
