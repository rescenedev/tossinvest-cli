...
def get_prices(client: ApiClient, symbols: list[str]) -> Any:
    """현재가 조회. 여러 종목을 콤마로 묶어 한 번에 조회."""
    return client.get("/api/v1/prices", params={"symbols": ",".join(symbols)})


def get_orderbook(client: ApiClient, symbol: str) -> Any:
    """호가 조회."""
    return client.get("/api/v1/orderbook", params={"symbol": symbol})


def get_trades(client: ApiClient, symbol: str, count: int | None = None) -> Any:
    """최근 체결 내역 조회."""
    return client.get("/api/v1/trades", params={"symbol": symbol, "count": count})


def get_price_limits(client: ApiClient, symbol: str) -> Any:
    """상/하한가 조회."""
